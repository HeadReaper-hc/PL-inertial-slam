...
public:
    ...
    void formLocalMap(std::deque<KeyFrame*> localKeyFrames);
    ...
    //sliding window
    std::mutex lk_sw;
    std::deque<KeyFrame*> slideWindow;
    int max_keyframe_in_slideWindow = 12;
    void addKeyframeToSW(KeyFrame* kf);
    void deleteKeyframeInSW();
    bool tryVioInit();
    Vector3d IMUInitEstBg(const std::deque<KeyFrame*> &vpKFs);

    //for imu
    Dataset* datasetPtr;
    inline void addDataset(Dataset* da){
        datasetPtr = da;
    }
    Vector3d gw;
    double gnorm = 9.8;
    ...
    vector<MapLine*> GetMapLines(){
        unique_lock<mutex> lock(mMapLineMutex);
        return map_lines;
    }

    vector<KeyFrame*> GetMapKeyFrames(){
        unique_lock<mutex> lock(mMapKeyFrameMutex);
...
private:
    ...
    std::mutex mMapLineMutex;
    std::mutex mMapKeyFrameMutex;

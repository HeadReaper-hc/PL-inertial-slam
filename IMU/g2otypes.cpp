 ...
 * @brief VertexLine
 */
 VertexLine::VertexLine() : BaseVertex<6, Vector6d>(){

 }

 void VertexLine::oplusImpl(const double* update_){
     Eigen::Map<const Vector6d> update(update_);
     _estimate += update;
 }

 /**
  * @brief EdgeNavStateLine
  */


 void EdgeNavStateLine::linearizeOplus(){
     ...
     Matrix<double,1,2> de_p(obs(0),obs(1));
     Matrix<double,2,3> dps_Ps, dpe_Pe;
     dps_Ps << fx / Pc_s(2), 0, -fx * Pc_s(0) / (Pc_s(2)*Pc_s(2)),
               0, fy / Pc_s(2), -fy * Pc_s(1) / (Pc_s(2)*Pc_s(2));

     dpe_Pe << fx / Pc_e(2), 0, -fx * Pc_e(0) / (Pc_e(2)*Pc_e(2)),
               0, fy / Pc_e(2), -fy * Pc_e(1) / (Pc_e(2)*Pc_e(2));

     Matrix<double,3,6> dPs_l, dPe_l;
     dPs_l.setZero();
     dPe_l.setZero();
     dPs_l.block<3,3>(0,0) = Rcb * Rwb.transpose();
     dPe_l.block<3,3>(0,3) = Rcb * Rwb.transpose();

     Matrix<double,3,6> de_l;
     de_l.setZero();
     de_l.block<1,6>(0,0) = de_p * dps_Ps * dPs_l;
     de_l.block<1,6>(1,0) = de_p * dpe_Pe * dPe_l;

     _jacobianOplusXi = de_l;

     Matrix<double,1,3> de0_Pwb, de1_Pwb, de0_fai, de1_fai;
     de0_Pwb = de_p * dps_Ps * (-Rcb * Rwb.transpose());
     de1_Pwb = de_p * dpe_Pe * (-Rcb * Rwb.transpose());
     de0_fai = de_p * dps_Ps * ( Rcb * skew(Rwb.transpose()*(pw_s-Pwb)));
     de1_fai = de_p * dpe_Pe * ( Rcb * skew(Rwb.transpose()*(pw_e-Pwb)));

     Matrix<double,3,9> de_nav;
     de_nav.setZero();
     de_nav.block<1,3>(0,0) = de0_Pwb;
     de_nav.block<1,3>(1,0) = de1_Pwb;
     de_nav.block<1,3>(0,6) = de0_fai;
     de_nav.block<1,3>(1,6) = de1_fai;
     _jacobianOplusXj = de_nav;
 }

 /**
  * @brief VertexLinePoint
  */
 void VertexLinePoint::oplusImpl(const double* update_){
     Eigen::Map<const Vector3d> update(update_);
     _estimate += update;
 }
  ...
  */
 void EdgeNavStateLinePoint::linearizeOplus(){
     ...
     Matrix3d Rcb = Rbc.transpose();
     Vector3d Pc_s = Rcb * Rwb.transpose() * (Pw - Pwb) - Rcb * Pbc;

     Vector3d obs(_measurement);

     Matrix<double,1,2> de_p(obs(0),obs(1));
     Matrix<double,2,3> dps_Ps;
     dps_Ps << fx / Pc_s(2), 0, -fx * Pc_s(0) / (Pc_s(2)*Pc_s(2)),
             0, fy / Pc_s(2), -fy * Pc_s(1) / (Pc_s(2)*Pc_s(2));

     Matrix<double,3,3> dPs_l;
     dPs_l= Rcb * Rwb.transpose();

     Matrix<double,1,3> de_l;
     de_l = de_p * dps_Ps * dPs_l;

     Matrix<double,3,3> jxi;
     jxi.setZero();
     jxi.block<1,3>(0,0) = de_l;
     _jacobianOplusXi = jxi;
     //std::cout<<"JXi: "<<_jacobianOplusXi<<std::endl;

     Matrix<double,1,3> de0_Pwb, de0_fai;
     de0_Pwb = de_p * dps_Ps * (-Rcb * Rwb.transpose());
     de0_fai = de_p * dps_Ps * ( Rcb * skew(Rwb.transpose()*(Pw-Pwb)));


     Matrix<double,1,9> de_nav;
     de_nav.setZero();
     de_nav.block<1,3>(0,0) = de0_Pwb;
     de_nav.block<1,3>(0,6) = de0_fai;

     Matrix<double,3,9> jxj;
     jxj.setZero();
     jxj.block<1,9>(0,0) = de_nav;
     _jacobianOplusXj = jxj;
     //std::cout<<"JXj: "<<_jacobianOplusXj<<std::endl;
        //debug
